...
class SingleShapeDataset(Dataset):
    def __init__(self,
                 ...
                 return_corr=True, return_dist=False, return_elas_evecs=False, bending_weight=1e-2, cache=True, return_dino=False, dino_folder="dino"):
        """
        ...
        Args:
            ...
            return_dino (bool, optional): Indicate whether return the precomputed DINO features of the shape. Default False.
        """
        # sanity check
        ...
        if self.return_dino:
            self.dino_dir = os.path.join(self.data_root, dino_folder)
            assert os.path.isdir(self.dino_dir), f'Invalid DINO feature directory: {self.dino_dir}.'
    ...
    def __getitem__(self, index):
        if self.cache:
            ...
            if self.return_dino:
                off_file = self.off_files[index]
                item['dino'] = self._load_dino_by_off(off_file)
            return item
        item = dict()
    ...
    def _load_dino_by_off(self, off_file, device="cpu"):
        basename = os.path.splitext(os.path.basename(off_file))[0]
        feat_path = os.path.join(self.dino_dir, f"{basename}.pt")
        if not os.path.exists(feat_path):
            raise FileNotFoundError(f"Missing dino feature: {feat_path}")
        return torch.load(feat_path, map_location=device).float()  # [V,768]
...
class SingleFaustDataset(SingleShapeDataset):
    def __init__(self, data_root,
                 ...
                 return_evecs=True, num_evecs=200,
                 return_corr=True, return_dist=False, return_elas_evecs=False, bending_weight=1e-2, cache=True, **kwargs):
        self.phase = phase
        super(SingleFaustDataset, self).__init__(data_root, return_faces,
                                                 return_evecs, num_evecs,
                                                 return_corr, return_dist, return_elas_evecs, bending_weight, cache, **kwargs)
...
class SingleShrec19Dataset(SingleShapeDataset):
    def __init__(self, data_root,
                 return_faces=True,
                 return_evecs=True, num_evecs=200,
                 return_dist=False, return_elas_evecs=False, bending_weight=1e-2, cache=True, **kwargs):
        super(SingleShrec19Dataset, self).__init__(data_root, return_faces, return_evecs, num_evecs, False, return_dist, return_elas_evecs, bending_weight, cache, **kwargs)
...
class SingleDT4DDataset(SingleShapeDataset):
    ...
    def _load_dino_by_off(self, off_file, device="cpu"):
        # override for dino features
        off_root = os.path.join(self.data_root, "off")
        rel = os.path.relpath(off_file, off_root)          # <cat>/<name>.off
        rel_pt = os.path.splitext(rel)[0] + ".pt"          # <cat>/<name>.pt
        feat_path = os.path.join(self.dino_dir, rel_pt)    # dino/<cat>/<name>.pt
        return torch.load(feat_path, map_location=device).float()
...
class SingleTopKidsDataset(SingleShapeDataset):
    def __init__(self, data_root, phase='train',
                 return_faces=True,
                 return_evecs=True, num_evecs=200, return_dist=False, return_elas_evecs=False, bending_weight=1e-2, cache=True, **kwargs):
        self.phase = phase
        super(SingleTopKidsDataset, self).__init__(data_root, return_faces,
                                                   return_evecs, num_evecs, False, return_dist, return_elas_evecs, bending_weight, cache, **kwargs)
...
class PairShrec19Dataset(Dataset):
    def __init__(self, data_root, phase='test',
        ...
        self.dataset = SingleShrec19Dataset(data_root, return_faces, return_evecs, num_evecs, return_dist, return_elas_evecs, bending_weight, cache, **kwargs)
        self.phase = phase
        if phase == 'test':
...
class PairTopKidsDataset(Dataset):
    def __init__(self, data_root, phase='train',
                 ...
                 return_dist=False, return_elas_evecs=False, bending_weight=1e-2, cache=True, **kwargs):
        assert phase in ['train', 'test'], f'Invalid phase: {phase}'
        self.dataset = SingleTopKidsDataset(data_root, phase, return_faces, return_evecs, num_evecs, return_dist, return_elas_evecs, bending_weight, cache, **kwargs)
        self.phase = phase
        if phase == 'test':
            corr_path = os.path.join(data_root, 'corres')
            assert os.path.isdir(corr_path), f'Invalid path {corr_path} not containing .vts files'
