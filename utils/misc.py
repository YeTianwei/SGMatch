...
def mkdir_and_rename(path):
    ...
    """
    if osp.exists(path) and osp.exists(osp.join(path,'log', 'ts.txt')):
        # rename it to the ts found in log folder
        with open(osp.join(path,'log', 'ts.txt'), 'r') as f:
            ts = f.readline()
            new_name = path + ts
        if osp.exists(new_name):
            new_name = path + get_time_str()
...
def make_exp_dirs(opt):
    ...
    if opt['is_train']:
        ...
        os.makedirs(path_opt['log'], exist_ok=True)
        os.makedirs(path_opt['pcks'], exist_ok=True)
        os.makedirs(path_opt['heatmaps'], exist_ok=True)
